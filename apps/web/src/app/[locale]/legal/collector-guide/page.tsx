...
/**
 * Loads locale-specific collector copyright & sharing markdown. Falls back to
 * Korean when a translation file is missing.
 *
 * ISO 27001 A.18.1.4 — transparency of published guidance (KO/JA/EN).
 * KO: 공개 가이드 문서를 로케일별로 제공하고, 없으면 정본(ko)으로 대체한다.
 * JA: 公開ガイドをロケール別に提供し、未整備時は正本(ko)にフォールバックする。
 * EN: Serves the public guide per locale, falling back to the Korean master when missing.
 */
async function loadCollectorGuideMd(
  locale: Locale,
  ...
  for (const loc of tryOrder) {
    ...
    try {
      const filePath = path.join(
        ...
        "public",
        "docs",
        `collector-copyright-guide.${loc}.md`,
      );
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: d.meta.collectorGuideTitle,
    description: d.meta.collectorGuideDescription,
  };
}

export default async function LegalCollectorGuidePage({ params }: Props) {
  const { locale: raw } = await params;
  const locale = normalizeLocale(raw);
  const m = getDictionary(locale);
  const p = m.legalCollectorGuide;
  const loaded = await loadCollectorGuideMd(locale);
