...
export function SubmissionReleaseMiniCard({
  locale,
  submissionId,
  title,
  artist,
  ...
  const href = withLocale(locale, `/releases/submission/${submissionId}`);
  const priceParts = priceJpy > 0 ? formatListPriceForLocale(locale, priceJpy) : null;
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/5] overflow-hidden rounded-md border border-white/[0.06] bg-gradient-to-b from-[#1f1f1f] to-opus-charcoal">
        <Image
          src={`/api/artwork-submissions/${submissionId}/public-preview`}
          alt={`${title} — ${artist}`}
          fill
          sizes="140px"
          unoptimized
